...
//     string flag = "left";
    ...
    //cout << tree->getData().first << " : " << tree->getData().second << endl;
    
    if(tree->left) 
        if(tree->left->getData().first != '/') {
            cout << "OLA LEFT" << endl << endl;
            aux << 0;
            data[tree->left->getData().first] += aux.str();
            aux.str("");
        }
        else
    ...
    if(tree->right)
        if(tree->right->getData().first != '/') {
            cout << "OLA RIGHT" << endl << endl;
            aux << 1;
            data[tree->right->getData().first] += aux.str();
            aux.str();
        }
    ...
    /*
    if(tree) {
>>>>>>> a4d8cbc663973b3ac11058889f7e7142cdf0162d
